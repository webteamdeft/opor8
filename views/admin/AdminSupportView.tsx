...
export const AdminSupportView: React.FC = () => {
  ...
  const updateConfig = (key: keyof GlobalConfig, val: string) => {
    DB.config.update({ [key]: val });
    setConfig(DB.config.get());
  };
                     ...
                     <div className="flex-1 overflow-y-auto space-y-4 mb-8 custom-scrollbar">
                        {selectedChat.messages.map(m => (
                          <div key={m.id} className={`flex ${m.isAdmin ? 'justify-end' : 'justify-start'}`}>
                             <div className={`max-w-[70%] p-6 rounded-[2rem] shadow-sm text-sm font-bold ${
                               m.isAdmin ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-700 border border-slate-100'
                             }`}>
                                {m.text}
                             </div>
                          </div>
