    ...
    <div className="p-10 max-w-5xl mx-auto space-y-12 font-sans">
      ...
      <div className="space-y-10">
        ...
        <section className="bg-white p-12 rounded-[3.5rem] border border-slate-200 shadow-sm space-y-10 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-48 h-48 bg-indigo-50 rounded-full translate-x-1/2 -translate-y-1/2"></div>
          <h3 className="text-2xl font-black text-slate-900 relative">Personal Information</h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">Full Name</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">Date of Birth</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">Gender</label>
              <select
        ...
        {/* Security Section */}
        <section className="bg-white p-12 rounded-[3.5rem] border border-slate-200 shadow-sm space-y-10 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-48 h-48 bg-emerald-50 rounded-full translate-x-1/2 -translate-y-1/2"></div>
          <h3 className="text-2xl font-black text-slate-900 relative">Security</h3>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">Current Password</label>
              <input
                type="password"
                value={oldPassword}
            ...
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">New Password</label>
              <input
                type="password"
                value={newPassword}
            ...
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 ml-2">Confirm New Password</label>
              <input
                type="password"
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <button
              ...
              className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-slate-800 transition-all shadow-lg flex items-center gap-3 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <section className="bg-white p-12 rounded-[3.5rem] border border-slate-200 shadow-sm space-y-10 relative overflow-hidden">
          ...
          <h3 className="text-2xl font-black text-slate-900 relative">Business Profile</h3>

          {/* Logo Management Section */}
          <div className="flex items-center space-x-8 relative">
            <div
              onClick={() => (user.isPaid || user.isPro) && fileInputRef.current?.click()}
              className={`w-28 h-28 rounded-[1.75rem] border-2 border-dashed border-slate-200 bg-slate-50 flex items-center justify-center transition-all overflow-hidden group shadow-inner ${(user.isPaid || user.isPro) ? 'cursor-pointer hover:border-indigo-500 hover:bg-indigo-50' : 'cursor-not-allowed opacity-50'
                }`}
            >
              {logoPreview ? (
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                <h4 className="text-xl font-bold text-slate-900">Brand Identity</h4>
                {!(user.isPaid || user.isPro) && <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-black uppercase tracking-widest">PRO FEATURE</span>}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {['Professional', 'Formal', 'Casual', 'Technical'].map(tone => (
                  <button
                    ...
                    className={`py-4 rounded-2xl border-2 text-sm font-black transition-all ${(localProfile.complianceTone || localProfile.tone) === tone ? 'border-indigo-600 text-indigo-600 bg-indigo-50 shadow-sm' : 'border-slate-100 text-slate-400 hover:border-slate-200 bg-slate-50/50'}`}
                  >
                    {tone}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 gap-4">
                {['PDF', 'DOCX', 'HTML', 'Markdown'].map(fmt => (
                  <button
                    ...
                    className={`py-4 rounded-2xl border-2 text-sm font-black transition-all ${(localProfile.primaryExportFormat || 'PDF') === fmt ? 'border-indigo-600 text-indigo-600 bg-indigo-50 shadow-sm' : 'border-slate-100 text-slate-400 hover:border-slate-200 bg-slate-50/50'}`}
                  >
                    {fmt}
                  </button>
                ))}
        ...
        <div className="space-y-4">
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="w-full px-10 py-5 bg-slate-900 text-white rounded-2xl font-black text-lg hover:bg-slate-800 shadow-xl transition-all flex items-center justify-center gap-3 active:scale-95"
          >
        ...
        <section className="bg-white p-12 rounded-[3.5rem] border-4 border-red-50 space-y-8">
          ...
          <p className="text-slate-500 text-lg leading-relaxed max-w-2xl font-medium">Permanently delete your account and all generated SOPs. <b>This action is irreversible.</b> All data will be purged from our AI compliance database immediately.</p>
