        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {steps.map((s, i) => (
              ...
              >
                <div className={`relative bg-white rounded-3xl p-8 border-2 transition-all duration-500 h-full flex flex-col ${
                  activeStep === i
                    ? 'border-indigo-600 shadow-2xl shadow-indigo-100 scale-105 -translate-y-2'
                    : 'border-slate-100 hover:border-slate-200 hover:shadow-lg'
                }`}>
                  {/* Step Number Badge */}
                  <div className={`absolute -top-4 -right-4 w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg transition-all duration-500 shadow-xl ${
                    activeStep === i
                  ...
                  {/* Icon Area */}
                  <div className={`w-16 h-16 rounded-2xl mb-6 flex items-center justify-center text-3xl transition-all duration-500 ${
                    activeStep === i
                  ...
                  {/* Progress Indicator */}
                  <div className={`h-1 bg-slate-100 rounded-full overflow-hidden transition-all duration-500 ${
                  ...
                  {i < 3 && (
                    <div className={`hidden lg:block absolute top-1/2 -right-3 w-6 h-6 transition-all duration-500 ${
                  ...
                  <div className="w-full max-w-lg animate-fadeIn">
                    <div className="flex items-center justify-center gap-8 mb-12">
                      {['📈 Sales', '⚙️ Operations', '👥 HR'].map((dept, idx) => (
                        <div
                          key={idx}
                          className={`w-28 h-28 bg-white border-2 rounded-2xl flex flex-col items-center justify-center shadow-lg transition-all duration-700 ${
                            idx === 1 ? 'border-indigo-600 scale-110 shadow-indigo-200' : 'border-slate-200'
                          }`}
                        ...
                        {[true, true, false, true].map((checked, idx) => (
                          <div key={idx} className="flex items-center gap-3">
                            <div className={`w-6 h-6 rounded-lg flex items-center justify-center font-bold text-xs ${
                              checked ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'
                            }`}>
                              {checked ? '✓' : '!'}
                            </div>
