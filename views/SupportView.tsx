             ...
             <div className="flex-1 overflow-y-auto p-8 bg-slate-50/30 space-y-6 custom-scrollbar">
                ...
                {session?.messages.map((m) => (
                  <div key={m.id} className={`flex ${m.isAdmin ? 'justify-start' : 'justify-end'} animate-fadeIn`}>
                    <div className={`max-w-[85%] p-5 rounded-[2.5rem] shadow-md text-sm font-bold leading-relaxed ${
                      m.isAdmin ? 'bg-white text-slate-900 border border-slate-200' : 'bg-indigo-600 text-white shadow-indigo-100'
                    }`}>
                      {m.text}
